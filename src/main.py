...
def process_page(page):
  ...
  for i in range(0, len(semesters_texts)):
    subjects, is_last_semester = process_semester(semesters_texts[i])
...
def main():
  ...
  for i in range(1, len(pages)): # a 1a pagina eh informacoes de curso
    page = pages[i]
    page_semesters_subjects, should_append_first_in_previous_page, has_ended = process_page(page)
  ...
  for i in range(len(semesters_subjects)): 
    print(f'semestre {i+1}')
    print([str(subject) for subject in semesters_subjects[i]])
    print()
